...
async function main() {
  try {
    ...
    if (process.platform === 'win32') {
      execSync(`powershell -Command "Compress-Archive -Path '${path.join(distDir, 'amana-server.exe')}', '${path.join(distDir, 'version.json')}', '${path.join(distDir, 'node.exe')}', '${path.join(distDir, 'server')}' -DestinationPath '${portableZipDest}' -Force"`);
    } else {
      execSync(`cd "${distDir}" && zip -r amana-hub-portable.zip amana-server.exe version.json node.exe server`);
    }
