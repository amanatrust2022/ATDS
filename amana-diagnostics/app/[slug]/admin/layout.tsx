...
import { RiDashboardLine, RiTeamLine, RiSettings3Line, RiHospitalLine, RiTestTubeLine, RiRadarLine, RiArrowLeftLine } from '@remixicon/react';
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { name: 'Workspace Overview', path: `/${slug}/admin`, icon: <RiDashboardLine size={20} /> },
    { name: 'Staff Management', path: `/${slug}/admin/staff`, icon: <RiTeamLine size={20} /> },
    { name: 'Organization Settings', path: `/${slug}/admin/settings`, icon: <RiSettings3Line size={20} /> },
  ];
          ...
          {navItems.map(item => {
            ...
            return (
              <Link key={item.path} href={item.path} style={{
                display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.75rem 1rem', borderRadius: 8,
