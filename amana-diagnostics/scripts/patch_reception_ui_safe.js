...
// 4. Fix Upgrade to Family Account Button - using EXACT index finding
const targetStr1 = `                          <button
                            type="button"
                            onClick={() => setShowAddExisting(prev => !prev)}`;
const targetStr2 = `                            {showQuickRegisterDep ? 'Close Register Form' : 'Register New Dependent'}
                          </button>
                        </div>`;

const idx1 = recContent.indexOf(targetStr1);
const idx2 = recContent.indexOf(targetStr2);
if (idx1 !== -1 && idx2 !== -1) {
  const startToIdx1 = recContent.substring(0, idx1);
  const afterIdx2 = recContent.substring(idx2 + targetStr2.length);
  
  const newButtons = `                          {showLedgerModal.type === 'individual' ? (
                            <button
                              type="button"
                              onClick={async () => {
                                try {
  ...
  recContent = startToIdx1 + newButtons + afterIdx2;
}

fs.writeFileSync(recPath, recContent, 'utf8');
console.log('Patched ReceptionPage.tsx perfectly');
