...
# Redian - Architectural Guidelines
...
- **Never `insert into T select * from jsonb_populate_record(null::T, p_row)`.** It materialises every column of `T`, so columns absent from the jsonb are inserted as explicit NULLs — and an explicit NULL **overrides the column DEFAULT**. This broke registration in production: the client omits `created_at` because the column has a default, and the insert hit its NOT NULL constraint. PostgREST does not behave this way; `.insert(row)` names only the keys present. Build the column list from the keys in the jsonb intersected with `pg_attribute`, and insert only those. See the correction note in `supabase_wallet_atomicity.sql`.
- **Declare a variable that will be written to a column as `table.column%TYPE`**, not `text`. A plpgsql assignment applies I/O conversion, so it works whether the column is `timestamptz` or `text`; an UPDATE target with a `text` variable does not. (`%TYPE` is fine in a DECLARE block — it is not valid in a parameter list or a GRANT signature.)
- **The fallback is keyed to "function missing" only.** `isMissingFunction` matches `PGRST202`/`42883`; every other error surfaces to the user. Do not widen it to catch constraint violations — a silent fallback there would mask a real defect and risk double-writing.
- **A missing optional column needs a narrow compatibility retry.** A function migration can legitimately lag an app release and send registration into the sequential fallback. If that fallback writes newer optional columns to an older table, retry only the exact PostgREST `PGRST204` schema-cache error for those named columns, omit only those fields, warn with the migration filename, and surface every other error. `insertPatientTestsCompat` is the reference pattern for package provenance.
- **Never delete visits to clear a duplicate-slip migration failure.** Keep the earliest holder of each slip, move only later visits to unused numbers above that day's current maximum, and update a matching registration-ledger `reference_id` by `patient_id` in the same transaction. Lock `patients`, verify that no duplicates remain, and build the unique index before committing. `supabase_repair_duplicate_slips.sql` is the production pattern.
- **SQL in this repo cannot be executed by an agent.** No Postgres, no Docker, no CLI in this environment, so anything written here is unverified until a human runs it. Give every SQL file a runnable VERIFY block that reproduces the specific failure it claims to fix, and a documented way to drop back to the fallback.
